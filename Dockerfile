...
# --- Stage 2: Build guest agent (Go) ---
FROM golang:1.23-bookworm AS go-builder
WORKDIR /build
COPY guest-agent/go.mod guest-agent/go.sum ./
RUN go mod download
COPY guest-agent/zypi-agent.go ./
RUN CGO_ENABLED=0 go build -o /zypi-agent -ldflags="-s -w" zypi-agent.go

# --- Stage 3: Runtime with Elixir + Firecracker ---
...
::sysinit:echo 1400 > /sys/class/net/eth0/mtu \
...
# Copy guest agent binary from Go builder (no pre-built host binary needed)
COPY --from=go-builder /zypi-agent /opt/zypi/bin/zypi-agent
