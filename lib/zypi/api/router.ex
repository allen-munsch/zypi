defmodule Zypi.API.Router do
  ...
  get "/.well-known/agent.json" do
    send_json(conn, 200, agent_card())
  end

  get "/.well-known/agent-card.json" do
    # yas-mcp compat: alias to /.well-known/agent.json
    send_json(conn, 200, agent_card())
  end
  ...
  get "/openapi.yaml" do
    spec_path = Path.join(:code.priv_dir(:zypi), "openapi.yaml")
    ...
    case File.read(spec_path) do
      ...
      {:error, _} ->
        send_json(conn, 404, %{error: "spec not found"})
    end
  end
...
get "/images/importing" do
  ...
  defp agent_card do
    %{
      name: "zypi",
      version: "0.2.0",
      description: "Firecracker microVM sandbox runtime — sub-second boots, CoW rootfs snapshots, one-shot execution, long-lived sessions, and image pre-warming",
      url: "https://github.com/allen-munsch/zypi",
      provider: %{
        name: "Zypi",
        url: "https://github.com/allen-munsch/zypi"
      },
      protocols: %{
        mcp: %{
          version: "2024-11-05",
          transports: ["http"],
          note: "MCP tools auto-generated from OpenAPI spec by yas-mcp proxy"
        },
        a2a: %{
          version: "1.0",
          agent_card_path: "/.well-known/agent.json",
          note: "Zypi is a runtime agent — tasks are sandbox exec/session operations delegated via A2A"
        }
      },
      auth: %{
        type: "none",
        note: "Internal cluster service. Auth handled at ingress/weft layer."
      },
      health: %{
        endpoint: "/health",
        interval_seconds: 10
      ...
      endpoints: %{
        ...
        agent_card: "http://localhost:4000/.well-known/agent.json",
      ...
      capabilities: %{
        exec: %{
          one_shot: true,
          streaming: true,
          description: "Execute commands in ephemeral Firecracker microVMs"
        },
        sessions: %{
          create: true,
          exec: true,
          ...
          list: true,
          stats: true,
          description: "Long-lived sandbox sessions for multi-step workflows"
        },
        images: %{
          list: true,
          import: true,
          delete: true,
          status: true,
          description: "OCI image management with CoW rootfs snapshots"
        },
        pool: %{
          warm: true,
          stats: true,
          description: "Pre-warm Firecracker VMs for sub-millisecond exec"
        },
        containers: %{
          create: true,
          start: true,
          ...
          list: true,
          logs: true,
          description: "Container lifecycle (backward-compatible API)"
        },
        grpc: %{
          available: true,
          port: 4001,
          services: "zypi.v1.ZypiService",
          description: "Typed protobuf interface for sandbox execution"
        }
      },
      default_skills: [
        %{
          name: "zypi_exec",
          description: "Execute a command in an ephemeral Firecracker microVM sandbox. Sub-second boot via CoW rootfs. Returns exit_code, stdout, stderr, duration_ms.",
          input_schema: %{
            type: "object",
            required: ["cmd"],
            properties: %{
              cmd: %{type: "array", description: "Command to execute (e.g., ['python', 'script.py'])", items: %{type: "string"}},
              image: %{type: "string", description: "OCI image to use (default: ubuntu:24.04)"},
              timeout: %{type: "integer", description: "Timeout in seconds (default: 60)"},
              env: %{type: "object", description: "Environment variables"},
              workdir: %{type: "string", description: "Working directory"},
              files: %{type: "object", description: "Files to inject (path -> content)"},
              ...
              stream: %{type: "boolean", description: "Enable SSE streaming of stdout/stderr"}
            }
          }
        },
        %{
          name: "zypi_session_create",
          description: "Create a long-lived Firecracker microVM session. Stays alive for multi-step workflows. Auto-expires after 5min idle.",
          input_schema: %{
            type: "object",
            properties: %{
              agent_id: %{type: "string", description: "Agent ID for attribution"},
              image: %{type: "string", description: "OCI image (default: ubuntu:24.04)"},
              vcpus: %{type: "integer", description: "vCPU count (default: 1)"},
              memory_mb: %{type: "integer", description: "Memory limit in MB (default: 256)"},
        ...
        %{
          name: "zypi_session_exec",
          description: "Execute a command in an existing Firecracker microVM session. Much faster than one-shot exec for multi-step workflows.",
          input_schema: %{
            type: "object",
            required: ["session_id", "cmd"],
            properties: %{
              session_id: %{type: "string", description: "Session ID from zypi_session_create"},
              cmd: %{type: "array", description: "Command to execute", items: %{type: "string"}},
        ...
        %{
          name: "zypi_image_warm",
          description: "Pre-warm Firecracker VMs for an image. Subsequent exec calls with matching image will be sub-millisecond.",
          input_schema: %{
            type: "object",
            required: ["ref"],
            properties: %{
              ref: %{type: "string", description: "Image reference (e.g., ubuntu:24.04)"},
              count: %{type: "integer", description: "Number of VMs to pre-warm (max: 10)"}
            }
          }
        },
        %{
          name: "zypi_pool_stats",
          description: "Get VM pool statistics — total VMs, warm/active counts, per-image breakdown.",
          input_schema: %{
            type: "object",
            required: [],
            properties: %{}
