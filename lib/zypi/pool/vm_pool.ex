      ...
      :ok ->
        state = case Map.get(state.vms, vm_id) do
          ...
          vm ->
            ...
            # Re-register in per-image pool so find_warm_vm can find it
            image_pools = case vm.image do
              nil -> state.image_pools
              img -> Map.update(state.image_pools, img, [vm_id], &(&1 ++ [vm_id]))
            end
            %{state |
              vms: Map.put(state.vms, vm_id, vm),
              warm_queue: state.warm_queue ++ [vm_id],
