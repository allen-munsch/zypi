defmodule Zypi.Runtime.Firecracker do
  ...
  defp setup_tap(container_id, _ip) do
    ...
    # NAT: MASQUERADE VM subnet so outbound traffic reaches the internet
    System.cmd("iptables", ["-t", "nat", "-A", "POSTROUTING", "-s", "10.0.0.0/24",
                            "-j", "MASQUERADE"], stderr_to_stdout: true)

    # Clamp TCP MSS in BOTH directions to match TAP MTU (1400 → MSS 1360).
    # --clamp-mss-to-pmtu uses the bridge's MTU (1500) which doesn't match the
    # TAP device, so large TLS handshake packets get dropped → HTTPS hangs.
    mss_clamp = ["-p", "tcp", "--tcp-flags", "SYN,RST", "SYN",
                 "-j", "TCPMSS", "--set-mss", "1360"]
    System.cmd("iptables", ["-A", "FORWARD"] ++ mss_clamp ++ ["-i", bridge_name],
               stderr_to_stdout: true)
    System.cmd("iptables", ["-A", "FORWARD"] ++ mss_clamp ++ ["-o", bridge_name],
               stderr_to_stdout: true)
  ...
  defp cleanup_network_rules(_container_id, _ip, tap) do
